...
#define ADXL345_MODE_STREAM 0x02 	//0010
...
public:
	...
	ADXL345(int CS);
	void powerOn();
	void readAccel(int* xyx,byte samples);
	void readAccel(int* x, int* y, int* z,byte samples);
	void get_Gxyz(double *xyz,byte samples);
//*********** lo que agregué!*******************************
	void setMode(byte modeBit); // setting operation mode
    byte getMode();
	byte getFifoEntries(void);
//**********************************************************
	...
	void setImportantInterruptMapping(int data_ready,int watermark,int single_tap, int double_tap, int free_fall, int activity, int inactivity);
  ...
  void DataReadyINT(bool status);
	...
	void setWatermark(byte wtrmrk);
	byte getRangeSetting(void);
