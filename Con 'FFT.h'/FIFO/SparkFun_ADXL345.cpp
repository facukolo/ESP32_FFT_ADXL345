...
void ADXL345::powerOn() {
    ...
    Serial.print("Tasa de transferencia I2C (Hz):");Serial.println(Wire.getClock());
	}
	//ADXL345 TURN ON
...
void ADXL345::readAccel(int *xyz,byte samples){
	readAccel(xyz, xyz + 1, xyz + 2,samples);
}
...
void ADXL345::readAccel(int* x, int* y, int* z, byte samples) {
...
void ADXL345::get_Gxyz(double *xyz,byte samples){
	int i;
	int xyz_int[3];
	readAccel(xyz_int,samples);
	for(i=0; i<3; i++){
...
byte ADXL345::getRangeSetting(void) {
	byte _b;
...
void ADXL345::setWatermark(byte watermark) {
    ...
    //setRegisterBit(ADXL345_FIFO_CTL, 6, operationMode);
    writeTo(ADXL345_FIFO_CTL, _b);
}// si quiero el nivel del agua en 32 muestras, debo mandarle 31?
...
void ADXL345::setImportantInterruptMapping(int data_ready,int watermark,int single_tap, int double_tap, int free_fall, int activity, int inactivity) {
	if(data_ready == 1) {
